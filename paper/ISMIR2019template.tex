...
\def\authorname{Oriol Nieto, Matthew McCallum, Matthew Davies, Andrew Robertson, Adam Stark, Eran Egozy}
...
madmom~\cite{Bock2016b} or librosa python libraries\footnote{We used madmom 0.16.1 and librosa 0.6.3. Note that we noticed a bias in this librosa version where beats were offset by a consistent number of milliseconds. To be specific, we evaluated with the librosa beat.beat_track method with default arguments on macOS 10.13.6}. By running these algorithms in other datasets with the same metrics, a comparison of datasets could ultimately be performed. The results are also provided with the dataset in CSV format. This is intendended
