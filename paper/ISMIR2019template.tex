...
These three musical characteristics are often related: downbeats define the first beat of a given music measure, and long structural music segments tend to begin and end on specific beat locations --frequently on downbeats. \MD{can you include a ref for this?}
...
The first time this task appeared on MIREX was in 2009,\footnote{https://www.music-ir.org/mirex/wiki/2009:Structural\_Segmentation} where annotations from The Beatles dataset (included in the set described above \MD{this isn't very clear, maybe just reference the system again?}) and a subset of the Real World Computing Popular Music Database (RWC)~\cite{Goto2002} were employed.
...
Some of these (Classic Rock, Country, Metal) may have less stable tempo (where drums are played by \MD{real?} musicians as opposed to drum-machines or DAW-based productions) and may deviate from a strict 4/4 meter. 
...
Unfortunately the availability of downbeat estimation libraries is limited. In order to provide a baseline for downbeat detection performance
...
of reliable beat tracking for downbeat estimation performance, and the interdependence between the beat tracking and downbeat estimation tasks. \MD{It's really not very clear what has been done here. Is it the same underlying method for downbeat estimation using the annotated beat locations and then fully automatic, or two different methods? If it's the former, I'd recommend changing Figure 9 so that it matches Figure 10 in this respect.}

\begin{figure}
    \centerline{\includegraphics[width=\columnwidth]{figs/2019_04_09_10_27_35_Downbeat_Re_F-Measure.pdf}}
    \caption{Downbeat tracking performance over the \setName~set, for the algorithms B{\"o}ck 1 \cite{Bock2016a} and B{\"o}ck 2 - a dynamic Bayesian network provided within the Madmom package \cite{Bock2016b}. \MD{There is a B{\"o}ck 1 and a B{\"o}ck 2 but these use the same notation for different systems to those in Figure 8 - I think!}}
...
Given how related boundary retrieval is with respect to precise beat placement, it is not unexpected to see a significant improvement in the boundary metrics (Hit Rates) when using annotated beats compared to those using estimated ones. \MD{Beware, ``significant'' is a catch word which can cause reviewers to question the lack of an accompanying statistical test.}
...
On the other hand, it is worth noting that the label results do not seem to depend as much on the quality of the beats in order to produce their outcomes, as the two different runs yield similar results. \MD{I think this is a super interesting result!!}
...
\MD{The references should be more uniform in their reporting of full names vs initials and concerning the names of the conferences etc. For the names there is a one line hack in the style file to allow a different bibliographystyle file}
